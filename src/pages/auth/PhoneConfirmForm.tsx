import React, { lazy, useState, useEffect, useRef, MouseEvent } from 'react';
...
import { fetchConfirmPhone, fetchSmsRepeat } from '../../store/api-actions';
...
export const cnPhoneConfirmForm = cn('PhoneConfirmForm');
export type FormValues = {smsCode:string};

const InputTextTypeSmsCode = withInputTextTypeSmsCode (InputText);
const RegistrationOk = lazy(() => import('./RegistrationOk'));

function PhoneConfirmForm(): JSX.Element {
  ...
  const handleSmsRepeat = (ev: MouseEvent) => {
    ev.preventDefault();
    dispatch(fetchSmsRepeat({phoneConfirmToken}));
  ...
  return (
    <div>
      {isRegistrationOk ? <RegistrationOk/> :
        <div className={cnPhoneConfirmForm(null, ['Auth', 'Form', 'Card'])}>
          <FormLogo/>
          ...
          <div>
            ...
            <div className="FormWarning">
              ...
              <p>Неподтвержденная регистрация будет удалена </p>
              <p>в течение {notConfirmedUserLifetime} суток</p>
            </div>
          </div>
          <Form
